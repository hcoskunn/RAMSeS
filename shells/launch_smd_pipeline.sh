#!/bin/bash
################################################################################
# launch_smd_pipeline.sh - RAMSeS SMD Pipeline Launcher
#
# This script:
#   1. Changes to RAMSeS directory
#   2. Activates RAMS conda environment
#   3. Runs the complete pipeline for SMD dataset only
#
# Usage:
#   ./launch_smd_pipeline.sh
#
# Or from anywhere:
#   /home/maxoud/local-storage/projects/RAMSeS/launch_smd_pipeline.sh
#
...
# Run SMD pipeline
./shells/run_complete_smd_pipeline.sh
...
if [ $exit_code -eq 0 ]; then
    echo ""
    echo "════════════════════════════════════════════════════════════════════════════════"
    echo "  ✅ SMD PIPELINE COMPLETED SUCCESSFULLY!"
    echo "════════════════════════════════════════════════════════════════════════════════"
else
    echo ""
    echo "════════════════════════════════════════════════════════════════════════════════"
    echo "  ❌ SMD PIPELINE FAILED (exit code: $exit_code)"
