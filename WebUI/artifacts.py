    ...
    {"id": "lints", "title": "Linear Thompson Sampling",
     "stages": ("thompson_ranking", "thompson_sampling"), "blocks": (
        ...
        {"id": "lints-dynamics", "title": "The reward split and the selection states", "blocks": (
            ...
            {"text": "The run is then divided into regimes, read straight off "
                     "the per-window leader. A threshold "
                     "governs both ends of a regime: a regime begins where a detector "
                     "has held the highest estimated expected reward for three "
                     "consecutive windows, it tolerates shorter interruptions, "
                     "and it closes at the last window that detector led once it "
                     "has gone three consecutive windows without leading. Windows belonging "
                     "to no regime are reported as contested, meaning the lead "
                     "changed hands there without anyone holding it long enough "
                     "to qualify. Because short interruptions are tolerated, "
                     "each regime also reports the share of its own windows "
                     "in which its leader actually held the lead.  "
                     "A window in which two detectors tie for the "
                     "highest estimated expected reward has no leader at all, "
                     "which is the case for every window before the first "
                     "posterior update, since every mean starts at zero. "},
            {"text": "These regimes are not the same thing as the previous "
                     "stage's streaks. Both are read off their own signal "
                     "directly, but a streak is any run of consecutive windows "
                     "under one leader, while a regime imposes a three-window "
                     "minimum at both ends and leaves the windows that fail it "
                     "contested. The two quantities and the two methods both "
                     "differ, so the two need not line up."},
