...
import { $, $$, el, getJSON, proseNode, familyClass, timeAgo, postJSON,
         makeSortable, tildeNodes } from "./dom.js";
...
/* Stages whose figures are held to the prose measure rather than the card width.
 *
 * A lone near-square figure stretched to the full card reads as a diagram the
 * card is built around, and towers over the paragraph that introduces it. */
const MEASURED_FIGURES = new Set(["rank_aggregation_final"]);
 ...
 * estimated expected reward into regimes, Ranking segments ‖μ‖² into streaks. */
...
function stageCard(stage, payload) {
  ...
  if (group.headline && group.headline.length) {
    const figures = el("div", {
      class: MEASURED_FIGURES.has(stage.key) ? "figures figures-measured" : "figures",
    }, group.headline.map(figureNode));
  ...
  // Provenance only, and dim: this is a property of the prose rather than part
  // of it. The row is dropped entirely when there is nothing to say.
  const provenance = [
...
function appendix(payload) {
  const rows = payload.stages.map((s) => el("tr", {},
    el("td", { text: s.title }),
  ...
  return el("details", { class: "card" },
    el("summary", { text: "Appendix: provenance" }),
    el("div", { class: "stack" },
      table,
